...
def my_rotate!(array, amt) 
  ...
  array.each_index do |i| 
    idx = (amt + i) % array.length  
    puts idx 
    shifted_indicies << array[idx]  
  end

  array.each_index do |i|
    array[i] = shifted_indicies[i]
  end  
...
def my_rotate!(array, amt)
  if amt > 0
    amt.times do 
      el = array.shift # .shift removes the first element of the array, but also returns it
      array << el # shuvel adds el to the end of the array
    end
  else  
    (-amt).times do # adding () around -amt makes it positive, which .times requires 
      el = array.pop 
      array.unshift(el)
    end 
  end 
