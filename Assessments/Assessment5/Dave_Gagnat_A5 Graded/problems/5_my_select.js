...
// no need to use myForEach as a helper here
// you already have a for loop inside of mySelect which is perfect for iterating -AZ

function mySelect(arr, cb) {
  var newArray = [];

  for (var i = 0; i < arr.length; i += 1) {
    var elm = arr[i]; // you don't do anything with the element of the array -AZ
    var values = myForEach(arr);

    // we need to pass the `elm` into the callback -AZ
