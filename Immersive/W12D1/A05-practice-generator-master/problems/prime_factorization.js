...
function primeFactorization1(num) {
  ...
  return [i].concat(primeFactorization1(num / i));
}
...
function primeFactorization2(num) {
  ...
  for (let i = 2; i <= upperRange; i += 1) {
    if (num % i === 0) {
      let factors = [i].concat(primeFactorization2(Math.floor(num / i)));
      return factors;
    }
  }
