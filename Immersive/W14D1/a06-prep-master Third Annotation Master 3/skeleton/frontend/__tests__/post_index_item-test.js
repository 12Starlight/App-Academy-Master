...
describe('post index item', () => {
  ...
  const testProps = {
    deletePost, // fakse props
    post
  };

  postIndexWrapper = mount(
    <MockRouter>
        {/* we are giving it some fake props */}
  ...
  it('has a button to delete post', () => {
    const deleteButton = postIndexWrapper.find('button').filterWhere(button =>
      // delete needed to be in between the button tags
      // button.props().children means between open and closing tags
