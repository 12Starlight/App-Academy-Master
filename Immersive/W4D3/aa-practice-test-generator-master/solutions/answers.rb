...
def longest_palindrome(sequence) # 3 times 
  largest = false 
  i = 0    
  ...
  while i < sequence.length - 1 
    ...
    while w < sequence.length 
      curr_seq = sequence[i..w]
      len = curr_seq.length 

      if is_palindrome?(curr_seq)
        largest = len if !largest || len > largest 
      end 
...
def merge_sort(array, &prc) # 3 times 
  prc ||= Proc.new { |a, b| a <=> b }
...
# 28
  ...
  def my_controlled_flatten(level = nil) # 3 times 
...
class Array
  ...
  def my_inject(accumulator = nil, &block) # 3 times 
    arr = self 
...
class Hash
  ...
  def my_merge(other_hash) # 3 times 
    dupped = self.dup 
...
# the original array
class Array
  def my_rotate(positions = 1) # 3 times
...
def prime_factorization(num) # 3 times
...
class String
  def real_words_in_string(dictionary) # 3 times
    real = [] 
...
def string_include_key?(string, key) # 3 times 
...
=begin    
All Methods 
  * 1 anagrams(str1, str2)
  * 2 base_converter(num, b), recursion 
  * 3 bsearch(array, target), recursion
  * 4 Array#my_bsearch(target), recursion
  * 5 Array#bubble_sort
  
  * 6 Array#bubble_sort(&prc)
  * 7 caesar_cipher(str, shift) 
  * 8 deep_dup(arr), recursion
  * 9 digital_root(num), recursion 
  * 10 dubler(array) # 3 times  
  
  * 11 Array#dups
  * 12 exponent(b, n), recursion
  * 13 factorials_rec(num), recursion
  * 14 factors(num)
  * 15 fibs_sum(n), recursion 
  
  * 16 first_even_numbers_sum(n), recursion
  * 17 jumble_sort(str, alpha = nil)
  * 18 longest_palindrome(sequence)
  * 19 Array#median
  * 20 merge_sort(array), recursion
  
  * 21 merge_sort(array, &prc), recursion
  * 22 Array#merge_sort(&prc), recursion
  * 23 Array#my_all?(&prc)
  * 24 Array#my_any?(&prc)
  * 25 Hash#my_each(&prc) 
  
  * 26 Array#my_each(&prc)
  * 27 Array#my_each_with_index(&prc)
  * 28 Array#my_flatten, recursion
  * 29 Array#my_controlled_flatten(level = nil), recursion 
  * 30 Array#my_inject(accumulator = nil, &block)
  
  * 31 Array#my_join(seperator = "")
  * 32 Hash#my_merge(other_hash)
  * 33 Array#my_reject(&prc)
  * 34 Array#my_reverse
  * 35 Array#my_rotate(postions = 1)
  
  * 36 Array#my_select(&prc)
  * 37 Array#my_zip(*arrays)
  * 38 prime_factorization(n), recursion
  * 39 primes(num)
  * 40 Array#my_quicksort(&prc),recursion
  
  * 41 String#real_words_in_string(dictionary)
  * 42 rec_sum(nums), recursion
  * 43 string_include_key?(string, key)  
  * 44 String#symmetric_substrings
  * 45 titleize(title)
  * 46 Array#two_sum


I David Robert Gagnat will graduate from App Academy November 1st, 2019 and work in my feid
