...
def top_titles
  ...
  # when putting your column name in a string, just use the name of the column
  # the name of the column can be used as a token --> name: 
  Movie 
    .select(:id, :title)
    .where('score >= 9')
end
...
def below_average_years
  ...
  # when using a token, make sure you match all tokens for the query method
  # for expressions use '' string 
  Movie 
