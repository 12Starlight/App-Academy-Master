...
def chris_is_missed
  ...
  # SELECT mov.title, new_actors.name
  # FROM actors 
  # JOIN castings AS ali_castings ON actors.id = ali_castings.actor_id
  # JOIN movies AS mov ON mov.id = ali_castings.movie_id
  # JOIN castings ON mov.id = castings.movie_id
  # JOIN actors AS new_actors ON new_actors.id = castings.actor_id
  # WHERE actors.name = 'Chris Farley' AND castings.ord = 1;